...
const CheckoutLocation = (props) => {
  ...
  const addCustomerDetailsChange = (e) => {
    ...
    let data = Object.assign({}, addAddressData);
    ...
    // console.log('Add Customer Form Data', data)
  ...
  const submitNewAddress = () => {
    ...
    const addUpdateData = {
      customerAddress: {
        ...
        street: streetArr,
        country_id: addAddressData?.country_id,
        custom_attributes: [
          {
            value: "",
    ...
    // console.clear();
    // console.log('Add Customer Address', addUpdateData)
    ...
    if (!addAddressData.country || addAddressData.country === '') {
    ...
    <div className='checkout-location'>
      ...
      {!showAddAddressForm && !showEditAddressForm &&
        <div className="select-address">
          ...
          {showAddressList && addressDetails.map((address, idx) => {
            return <div key={idx} className="selected-address card">
              ...
              <div className="addressactions">
                ...
                <button className='btn btn-sm btn-light text-danger' onClick={() => appData.deleteShippingAddress(address.id)}><FaTrashAlt /></button>
              </div>
            </div>
          })
          }
          ...
          {showAddAddressMap &&
            <div className="select-address-map">
              ...
              {!mapLoading && <div className="map">
                ...
                />
                <button className='btn btn-primary  mt-3 w-100' onClick={() => handleMapAddressDetails()}>Address Details</button>
              </div>
              }
            </div>
          }
          ...
          {!showAddAddressMap &&
            <button className='btn btn-outline-secondary w-100' onClick={() => handleAddAddress()}>Add Address</button>
          }
        </div>
      }
      ...
      {showAddAddressForm &&
        <div className="select-address-details">
          ...
          <button className='btn btn-primary w-100' onClick={() => submitNewAddress()}>Add Address</button>
        </div>
      }
...

      {
        showShippingMethods && <div className='shipping-methods '>
          <div className="card-body">
                ...
                return <div key={idx} className="form-check">
                  <label className="form-check-label">
                    <input
                      ...
                      checked={selectedMethod == option.method_code ? true : false}
                    />
            ...
            {shippingMethods && shippingMethods?.length === 0 && <p>Shipping method not found.</p>}
            <button disabled={selectedMethod === '' ? true : false} className='btn btn-primary w-100' onClick={() => handleSelectedShipping()}>OK</button>
          </div>
        </div>
      }
    </div>
