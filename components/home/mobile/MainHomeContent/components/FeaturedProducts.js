...
export default function FeaturedProducts(props) {
  ...
  const imgUrl = 'https://newness.net/pub/media/catalog/product/'
      ...
      {data.length > 0 &&
        <div className={styles.featuredProducts}>
          <div className={styles.featuredProductsHeader}>
            ...
            <Link href={{pathname: '/products', query:{pCatId: activeCategoryId}}}><button>View All</button></Link>
          </div>
          <Swiper
            slidesPerView={2.3}
            spaceBetween={5}
              ...
              <SwiperSlide key={item.id}>
                <div className={styles.card}>
                  <Link href={`/product/${item.sku}`}>
                    <Image src={item.extension_attributes.prod_image_url} alt={item.name} width={150} height={150} />
                  </Link>
                  <h3>{item.name}</h3>
                  <p>
